...
class ArcballControls extends EventDispatcher {
    ...
    constructor(camera, domElement, scene) {
        ...
        //2023/05/06 update to pan viewport
        this._viewportPan = new Vector2();
        ...
        this._panAction = 'single';
        this.MOUSE_PRESS_RIGHT = false;
        this.MOUSE_PRESS_LEFT = false;
    ...
    onMouseDown = (event) => {
        if (event.button == 2) { this.MOUSE_PRESS_RIGHT = true; }
        if (event.button == 0) { this.MOUSE_PRESS_LEFT = true; }
    }

    onMouseUp = (event) => {
        if (event.button == 2) { this.MOUSE_PRESS_RIGHT = false; }
        if (event.button == 0) { this.MOUSE_PRESS_LEFT = false; }
    }
        ...
        if (this.enabled) {
            ...
            switch (operation) {
                ...
                case 'PAN':
                    ...
                    if (this._panAction == 'both')
                        if (!this.MOUSE_PRESS_LEFT || !this.MOUSE_PRESS_RIGHT)
                            return;
    ...
    onSinglePanMove = (event, opState) => {
        ...
        if (this.enabled) {
            ...
            let restart = opState != this._state;
            this.setCenter(event.clientX, event.clientY);
                ...
                case STATE.ROTATE:
                    ...
                    if (this.enableRotate) {
                        ...
                        if (restart) {
                            ...
                            const amount = Math.max(distance / this._tbRadius, angle) * this._rotateSpeed; //effective rotation angle
    ...
    setMousePanAction = (type) => {
        this._panAction = type;
        if (type == 'both') {
...
/*
        let dx, dy;
        let xl = (window.innerWidth - this.domElement.clientLeft) / window.innerWidth;
        let yl = (window.innerHeight - this.domElement.clientTop) / window.innerHeight;

        dx = (this._currentCursorPosition.x - this._startCursorPosition.x) * xl;
        dy = (this._currentCursorPosition.y - this._startCursorPosition.y) * yl;

        this._viewportPan.x += dx;
        this._viewportPan.y += dy;

        console.log(dx,dy);

        this.camera.setViewOffset(
            this.domElement.clientWidth, this.domElement.clientHeight,
            -this._m4_1.elements[12],
            -this._m4_1.elements[13],
            this.domElement.clientWidth, this.domElement.clientHeight
        );
*/
    ...
    scale = (size, point, scaleGizmos = true) => {
        ...
        if (true) {//this.camera.isOrthographicCamera
